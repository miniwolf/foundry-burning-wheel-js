...
export async function CreateBurningWheelMacro(data, slot) {
    ...
    if (macroData) {
        ...
        let macro = game.macros?.contents.find(m => (m.name === macroData.name) && (m.system.command === macroData.command));
        if (!macro) {
...
function CreateItemMacro(data) {
    const itemType = data.system?.type || "";
