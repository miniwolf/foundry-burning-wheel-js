...
function updateItem(item, updateInfo) {
    const data = {};
    if (item.system.img === "icons/svg/item-bag.svg") {
        if (item.system.type === "skill") {
            data["img"] = skillImages[item.system.skilltype];
        }
        else {
            data["img"] = traitImages[item.system.traittype];
        }
    }
