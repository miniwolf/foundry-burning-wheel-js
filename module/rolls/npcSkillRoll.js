...
export async function handleNpcSkillRoll({ actor, skill, extraInfo, dataPreset }) {
            ...
            return new Dialog({
                ...
                buttons: {
                    ...
                    root2: {
                        ...
                        callback: () => {
                            const stat2 = getProperty(actor.system, `data.${skill.system.root2}`);
                            rollData.dice = stat2.exp;
                            rollsystem.shade = stat2.shade;
                            rollData.open = stat2.open;
                            rollData.statName = skill.system.root2;
    ...
    const data = mergeDialogData({
        ...
        forkOptions: actor.getForkOptions(skill.system.name).sort(byName),
        wildForks: actor.getWildForks(skill.system.name).sort(byName),
