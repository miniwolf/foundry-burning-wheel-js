...
export async function handleSkillRoll({ actor, skill, dataPreset, extraInfo, onRollCallback }) {
    ...
    const templateData = mergeDialogData({
        ...
        forkOptions: actor.getForkOptions(skill.system.name).sort(helpers.byName),
        wildForks: actor.getWildForks(skill.system.name).sort(helpers.byName),
...
async function skillRollCallback(dialogHtml, skill, actor, extraInfo) {
    ...
    if (success || actor.system.successOnlyRolls.indexOf(skill.name.toLowerCase()) === -1) {
