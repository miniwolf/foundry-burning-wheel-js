...
export async function handleNpcStatRollEvent({ target, sheet, dataPreset }) {
    ...
    const dice = getProperty(actor.system, target.dataset.stat || "");
    const shade = getProperty(actor.system, target.dataset.shade || "");
...
export async function handleNpcStatRoll({ dice, shade, open, statName, extraInfo, dataPreset, actor }) {
    ...
    const data = mergeDialogData({
        ...
        circlesBonus: statName === "circles" ? actor.system.circlesBonus : undefined,
        circlesMalus: statName === "circles" ? actor.system.circlesMalus : undefined,
        stat: { exp: dice },
        tax: 0,
