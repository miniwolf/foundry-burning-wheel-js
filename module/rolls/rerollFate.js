...
export async function handleFateReroll(target) {
    ...
    if (reroll) {
        ...
        if (actor.system.fate !== 0 && actor.system.type === "character") {
            const char = actor;
            if (target.dataset.rerollType === "stat") {
                ...
                if (learningTarget === 'skill') {
                    ...
                    if (successes <= obstacleTotal && success) {
                        if (actor.system.successOnlyRolls.includes(learningTarget)) {
                            actor.addStatTest(getProperty(actor, `data.system.${learningTarget}`), learningTarget.titleCase(), `data.${learningTarget}`, target.dataset.difficultyGroup, true);
                        }
                    }
