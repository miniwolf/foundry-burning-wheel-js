...
export async function handleSpellRollEvent({ target, sheet, dataPreset }) {
    ...
    if (!sorcerySkillId) {
        return helpers.notifyError("No Skill Specified", "A skill must be specified in order for the spell test to be rolled. Please pick from a list of sorcerous of the character.");
    }
...
export async function handleSpellRoll({ actor, spell, skill, dataPreset }) {
    if (!spell) {
        return helpers.notifyError("Missing Spell", "The spell being cast seems to be missing from the character sheet.");
    }
            ...
            if (spell.system.inPracticals) {
                ...
                if (amount + 1 >= aptitude) {
                    return Dialog.confirm({
                        ...
                        yes: () => {
                            spell.update({ "system.inPracticals": false, "system.learningProgress": 0 }, {});
                        },
                        no: () => void 0
                    });
                }
