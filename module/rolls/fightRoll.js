...
export async function handleFightRoll({ actor, type, itemId, attackIndex, positionPenalty, engagementBonus, dataPreset }) {
    ...
    if (type === "skill") {
        ...
        switch (item.type) {
            case "melee weapon":
            case "ranged weapon":
                if (item.system.type === "melee weapon" && typeof attackIndex === 'undefined') {
    ...
    if (actor.system.type === "npc") {
        // npc specific code
        const dice = parseInt(getProperty(actor, `data.system.${type}.exp`));
        const shade = getProperty(actor, `data.system.${type}.shade`);
