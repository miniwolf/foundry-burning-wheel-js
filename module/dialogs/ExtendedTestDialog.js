...
export const changesState = (callback) => {
    return function (_target, _propertyKey, descriptor) {
        ...
        descriptor.value = async function (...args) {
            ...
            await this.persistState(this.system);
            this.render();
...
export class ExtendedTestDialog extends Dialog {
    ...
    getData() {
        const data = Object.assign(super.getData(), this.system);
        return data;
    }
    ...
    activateSocketListeners() {
        game.socket.on(constants.socketName, ({ type, data }) => {
            if (type === `update${this.system.topic}`) {
                mergeObject(this.system, data);
            ...
            else if (type === `show${this.system.topic}`) {
                this.render(true);
            }
        });
    }
    _getHeaderButtons() {
        ...
        const showAllButton = {
            ...
            onclick: () => {
                game.socket.emit(constants.socketName, { type: `show${this.system.topic}` });
            }
        };
        if (game.user?.isGM) {
    ...
    syncData(data) {
        game.socket.emit(constants.socketName, { type: `update${this.system.topic}`, data });
    }
    async persistState(data) {
        game.settings.set(constants.systemName, this.system.settingName, JSON.stringify(data));
    }
}
__decorate([
    changesState()
