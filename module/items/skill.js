...
export class Skill extends BWItem {
    getRootSelect() {
        ...
        if (this.system.hasOwner && actor.system.type === "character") {
    ...
    async addTest(difficulty, force = false) {
        ...
        if (this.system.learning) {
            ...
            let shade = getProperty(this.actor || {}, `system.system.${this.system.root1.toLowerCase()}`).shade;
            this.update({ "system.learningProgress": progress + 1 }, {});
            if (progress + 1 >= requiredTests) {
                if (this.system.root2 && this.actor) {
                    const root2Shade = getProperty(this.actor, `system.system.${this.system.root2.toLowerCase()}`).shade;
                    if (shade != root2Shade) {
                        requiredTests -= 2;
                    }
                ...
                Dialog.confirm({
                    ...
                    yes: () => {
                        ...
                        updateData["system.exp"] = Math.floor(this.rootStatExp / 2);
    ...
    async _preUpdate(changed, options, userId) {
        ...
        if (changed.system?.skilltype && this.system.img === skillImages[this.system.skilltype]) {
